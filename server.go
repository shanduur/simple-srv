...
// AddSubRouter adds and returns subrouter to main server
func (srv *WebServer) AddSubRouter(path string) *mux.Router {
	srtr := srv.MainRouter.PathPrefix(path).Subrouter()
	srv.SubRouters[path] = srtr
